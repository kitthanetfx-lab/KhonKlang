...
/** ประกาศหาสินค้า — mobile shell: sticky ลงประกาศ · ฟอร์มใน sheet */
export function WantedApp({
  loading,
  myId,
  filtered,
  search,
  ...
  return (
    <AppPage withBottomNav>
      ...
      <AppFeed>
        <p className="app-lead" style={{ marginTop: 0 }}>
          ลงประกาศฟรี ผู้ขายทั้งระบบเห็นความต้องการของคุณ
        </p>

        <form
          className="wanted-app-search"
          role="search"
        ...
        {activeFilters > 0 && (
          <button
            type="button"
            className="wanted-app-clear"
            onClick={() => { onCat(''); onProvince(''); onMode(''); }}
          >
        ...
        {!loading && filtered.length === 0 && (
          <AppEmpty action={
            <button type="button" className="btn btn-primary" onClick={myId ? onToggleForm : onLoginToPost}>
              ลงประกาศแรก
            </button>
          }>
                ...
                <div className="wanted-app-actions">
                  ...
                  {!mine && (
                    <Link
                      ...
                      href={`/service/meetup?step=2&role=seller&title=${encodeURIComponent(p.title)}&wantedId=${p.id}&inviteUserId=${p.user_id}`}
                    >
                      🚗 นัดรับ
                    </Link>
                  )}
                  {!mine && (
                    <Link
                      ...
                      href={`/messages?to=${p.user_id}&name=${encodeURIComponent(p.user_name || 'สมาชิก')}`}
                    >
                  ...
                  {!mine && p.contact && (p.buy_mode === 'direct' || p.buy_mode === 'both') && (
                    contactOpen === p.id
                      ...
                      : (
                        <button type="button" className="btn btn-ghost btn-sm" onClick={() => onContactOpen(p.id)}>
                          ดูช่องทางติดต่อ
                        </button>
                      )
                  )}
                  {mine && (
                    <button type="button" className="btn btn-ghost btn-sm" onClick={() => onClosePost(p.id)}>
                      ได้ของแล้ว — ปิด
                    </button>
                  )}
                </div>
      ...
      <AppStickyBar>
        <button
          ...
          onClick={myId ? onToggleForm : onLoginToPost}
        >
      ...
      <AppSheet
        ...
        footer={
          <>
            <button type="button" className="btn btn-soft btn-lg" onClick={onToggleForm}>
              ยกเลิก
            </button>
      ...
      >
        <label className="app-field">
          กำลังหาอะไร? *
          <input value={fTitle} onChange={e => onFTitle(e.target.value)} maxLength={200} placeholder="เช่น iPhone 15 Pro 256GB" />
        </label>
        <label className="app-field">
          รายละเอียด
          <textarea value={fDetail} onChange={e => onFDetail(e.target.value)} rows={3} maxLength={1000} placeholder="สเปก เงื่อนไข ฯลฯ" />
        </label>
        <div className="wanted-app-form-grid">
          <label className="app-field">
            งบต่ำสุด
            <input type="number" min="0" value={fBudgetMin} onChange={e => onFBudgetMin(e.target.value)} placeholder="ไม่ระบุ" />
          </label>
          <label className="app-field">
            งบสูงสุด
            <input type="number" min="0" value={fBudgetMax} onChange={e => onFBudgetMax(e.target.value)} placeholder="ไม่ระบุ" />
          </label>
        </div>
        <label className="app-field">
          หมวดหมู่
          <select value={fCat} onChange={e => onFCat(e.target.value)}>
        ...
        <label className="app-field">
          จังหวัด
          <select value={fProvince} onChange={e => onFProvince(e.target.value)}>
            <option value="">ทุกจังหวัด</option>
            {PROVINCES.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
        </label>
          ...
          <div className="wanted-app-mode-grid">
            {(Object.keys(MODE_INFO) as ('middleman' | 'direct' | 'both')[]).map(m => (
              <button
                key={m}
                type="button"
                className={`wanted-app-mode${fMode === m ? ' is-on' : ''}`}
                onClick={() => onFMode(m)}
              >
        ...
        <label className="app-field">
          ...
          <input value={fContact} onChange={e => onFContact(e.target.value)} maxLength={200} placeholder="LINE / เบอร์โทร" />
        </label>
