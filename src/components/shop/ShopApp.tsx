...
export function ShopApp({ loading, error, shop, stats, listings, sold, imgUrl }: ShopAppProps) {
  ...
  return (
    <SubPageApp title={shop.shopName} backHref="/marketplace">
      <div className="shop-app-sign">
        ...
        <div className="shop-app-stats">
          ...
          <div>
            ...
            <span>
              {stats.reviewCount > 0
                ? `${stars(stats.reviewScore)} (${stats.reviewCount})`
                : 'ยังไม่มีรีวิว'}
            </span>
          </div>
        </div>
      ...
      {items.length === 0 ? (
        <AppEmpty>
          {tab === 'active' ? 'ยังไม่มีสินค้าในร้าน' : 'ยังไม่มีสินค้าที่ขายแล้ว'}
        </AppEmpty>
      ) : (
        <AppList>
          {items.map(item => {
            ...
            return isSold ? (
              <li key={item.id}>
                <div className="app-row shop-app-row--sold">
                  ...
                  <div className="app-row-body">
                    ...
                    {item.condition && (
                      <div className="app-row-meta">{item.condition}</div>
                    )}
                  </div>
                </div>
              </li>
              ...
              <AppListRow
                ...
                badge={
                  item.deal_type === 'auction' ? (
                    <span className="shop-app-badge shop-app-badge--auction">ประมูล</span>
                  ) : undefined
                }
