...
import type { MarketplaceBuyUiState } from '@/lib/marketplaceOrder';
import {
  AppPage,
  AppHeader,
  AppFeed,
...
export type MarketplaceDetailAppProps = {
  ...
  shippingCost: number;
  buyState: MarketplaceBuyUiState;
...
export function MarketplaceDetailApp(props: MarketplaceDetailAppProps) {
  const {
    loading, error, listing, sellerShop, images, displayImage, onMainImageChange,
    myId, isOwner, isAuction, auction, auctionBids, displayPrice,
    shippingProviders, shippingCost, buyState, selectedShipping, onSelectedShippingChange,
    joining, bidding, bidAmount, onBidAmountChange, maxBidAmount, onMaxBidAmountChange,
    bidStepAmount, onBidStepAmountChange, autoBidOn, onAutoBidToggle,
    myAutoBidMax, myAutoBidStep, hasLineNotify, lineOaUrl, bidError,
    onBuy, onBid, onSellerChat,
  } = props;
  ...
  const showAuctionBid = isAuction && !isOwner && auction && auction.phase === 'live';
  const showBuySticky = !isAuction && !isOwner && (buyState === 'can_buy' || buyState === 'continue_checkout');
      ...
      return (
        <div className="app-detail-sticky">
          <div className="app-detail-sticky-price">
            <span className="app-detail-sticky-lbl">{auction.bidCount > 0 ? 'ราคาปัจจุบัน' : 'ราคาเริ่ม'}</span>
          ...
          <div className="app-detail-sticky-row">
            ...
            <button type="button" className="btn btn-primary btn-lg app-detail-sticky-btn" onClick={onBid} disabled={bidding}>
              {bidding ? '…' : 'Bid'}
            </button>
          </div>
          <div className="app-detail-sticky-tools">
            <button
              ...
              onClick={() => { onAutoBidToggle(); if (!autoBidOn) setAutoBidSheetOpen(true); }}
      ...
      return (
        <div className="app-detail-sticky">
          <div className="app-detail-sticky-price">
            ...
            <strong className="app-detail-sticky-val">฿{displayPrice.toLocaleString()}</strong>
            {shippingCost > 0 && <span className="app-detail-sticky-ship">+ ขนส่ง ฿{shippingCost.toLocaleString()}</span>}
          </div>
          {buyState === 'continue_checkout' ? (
            ...
            <button type="button" className="btn btn-primary btn-block btn-lg" onClick={onBuy} disabled={joining}>
  ...
  return (
    <AppPage accent={isAuction ? 'auction' : 'default'} withBottomNav stickyFooter={stickyFooter}>
      <AppHeader title={isAuction ? 'ประมูล' : 'รายละเอียด'} backHref={backHref} />
      <AppFeed>
        <div className="app-detail-gallery-wrap">
          {images.length > 0 ? (
            <>
              <div ref={galleryRef} className="app-detail-gallery" onScroll={syncSlideFromScroll}>
                {images.map(src => (
            ...
            <div className="app-detail-slide app-detail-slide--empty"><Icon name="package" size={48} /></div>
          )}
        </div>
        ...
        {auction && (
          <div className="app-card app-detail-auction-stats">
            <div className="app-detail-stat"><span>เหลือเวลา</span><strong><AuctionCountdown endsAt={auction.endsAt} endedAt={auction.endedAt} liveClassName="is-live" /></strong></div>
            <div className="app-detail-stat"><span>ผู้ประมูล</span><strong>{auction.uniqueBidderCount} คน · {auction.bidCount} bid</strong></div>
            <div className="app-detail-stat"><span>นำอยู่</span><strong>{auction.currentBidderName || '— ยังไม่มี'}</strong></div>
            <div className="app-detail-stat"><span>บิทครั้งละ</span><strong>฿{auction.bidIncrement.toLocaleString()}</strong></div>
          </div>
        )}

        {auction && !isOwner && auction.phase !== 'live' && (
          <div className="app-card app-detail-note">
            <strong>{listing.buyer_id || auction.endedAt ? 'ขายแล้ว · ประมูลปิด' : 'ประมูลปิดแล้ว'}</strong>
        ...
        {listing.description && <div className="app-card app-detail-desc">{listing.description}</div>}
        ...
        {!isAuction && !isOwner && buyState === 'can_buy' && (
          <p className="app-detail-hint">กดซื้อแล้วระบบพาไปยืนยันที่อยู่และชำระเงิน — ผู้ขายจัดส่งตรงถึงคุณ</p>
        )}
            ...
            {hasLineNotify ? (
              ...
              <>
                ...
                <div className="app-detail-line-actions">
                  {lineOaUrl ? (
                    <a className="btn btn-soft btn-sm" href={lineOaUrl} target="_blank" rel="noreferrer">เพิ่มเพื่อน LINE OA</a>
                  ) : null}
                  <a className="btn btn-soft btn-sm" href={`/api/auth/line?returnTo=${encodeURIComponent(`/marketplace/${listing.id}`)}`}>เชื่อมด้วย LINE Login</a>
                </div>
              </>
            )}
          </div>
      ...
      {showAuctionBid && auction && (
        <AppSheet
          ...
          footer={
            <button type="button" className="btn btn-primary btn-block btn-lg" onClick={() => setAutoBidSheetOpen(false)}>
              บันทึกการตั้งค่า
            </button>
          }
        >
