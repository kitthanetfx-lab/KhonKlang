...
const CAT_LABEL: Record<string, string> = {
  it: 'ไอที/มือถือ', amulet: 'พระเครื่อง', luxury: 'แบรนด์เนม',
  vehicle: 'รถ', game: 'ไอดีเกม', appliance: 'เครื่องใช้ไฟฟ้า',
  jewelry: 'อัญมณี', general: 'ทั่วไป',
};
...
function TierBadge({ tier }: { tier: string }) {
  const cls = TIER_CFG[tier] ?? 'text-gray-700 bg-gray-50 border-gray-200';
  return (
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white dark:bg-gray-900 shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <h2 className="font-semibold">รายละเอียดใบสมัครคนกลาง</h2>
...
function MiddlemenContent() {
  ...
  const [apps, setApps]       = useState<MiddlemanApp[]>([]);
  ...
  const setStatus = (s: string) => {
    ...
    router.push(`/admin/middlemen?${params}`);
  };
  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2"><Shield size={20} /> ใบสมัครคนกลาง</h1>
          <p className="text-sm text-gray-500 mt-0.5">จัดการและอนุมัติใบสมัครคนกลาง</p>
        </div>
        <button onClick={() => load()}
