...
**แก้ 2026-08-11:** อัปโหลดรูปไม่ได้ — รองรับ mime ว่าง/HEIC, บีบอัดรูปก่อนส่ง, refresh session + แจ้งเข้าสู่ระบบ
...
**เป้าหมาย:** ไม่ย่อโครง Yahoo/เดสก์ท็อป · สร้าง shell แอปมือถือแยกทั้งก้อน (≤767px)  
**ใหม่:** `MarketplaceApp` + CSS `.mkt-app*` · แท็บซื้อขาย/ประมูล · ค้นหาแอป · chips หมวดเลื่อน · filter sheet · รายการแนวนอน 1 คอลัมน์  
**เทคนิค:** `.mkt-mobile-shell` / `.mkt-desktop-shell` สลับด้วย CSS · logic เดิมใช้ร่วม · desktop คงโครงเดิม · hook `useIsMobile` พร้อมใช้ต่อ  
**ขอบเขต:** เฉพาะ `/marketplace` list — ถ้าดีค่อยขยายทั้งเว็บ

---

## 2026-08-11 (00:42)

### ตลาดมือถือ — ลดขอบข้าง + ขยายช่องค้นหา/ปุ่ม

**ปัญหา:** `.mkt-container` ใช้ `1cm` ต่อข้าง → เหลือแถบขาวมาก · ช่องค้นหา/ปุ่มถูกบีบดูเล็ก  
**แก้:** มือถือ padding **12px** · แถบค้นหาสูง **48px** · แท็บ/ปุ่มมุมมอง **44px** · ฟอนต์ input 16px (กัน iOS zoom)

---

## 2026-08-11 (00:29)

### ตลาด/ประมูลมือถือ — จัดรูปแบบใหม่ตาม UI/UX

**เป้าหมาย:** ลดความรก ฟอนต์อ่านง่าย เห็นสินค้าเร็ว · ไม่แตะ desktop  
**Hero:** แท็บโซนเต็มแถว · ซ่อน sub/CTA/brand ซ้ำ · ซ่อนชิปหมวด 8 ตัว (เหลือ dropdown)  
**การ์ด:** ชื่อ 14px · ราคา 16px · ประมูลสถิติ 2 ช่อง (ราคา+เวลา) ซ่อนผู้นำ/จังหวัด  
**กริดประมูล:** ล็อก 2 คอลัมน์ถึงแท็บเล็ต (เลิก 3 คอลที่ 480px)
...
**UI:** ลำดับสายตา (เพดานก่อนปุ่ม) · hierarchy/contrast ช่องเพดานขอบม่วงหนา · proximity กับปุ่มเปิด Auto-bid  
**UX:** progressive disclosure ยังเปิดค่อยโชว์ · อ่านเงื่อนไขก่อนกด Bid  
ลำดับใหม่: เปิด Auto-bid → ช่องเพดาน → `[ราคา bid][Bid]` → แชท
...
บันทึกใน `.cursor/rules/ui-ux-principles.mdc` (`alwaysApply: true`)  
ครอบคลุม UI 7 ข้อ + UX 7 ข้อ — ต้องปฏิบัติทุกครั้งที่ออกแบบ/แก้หน้าจอ

---

## 2026-08-10 (18:58)

### หน้ารายละเอียดประมูล — ย่อส่วน Bid + ปุ่มเปิด Auto-bid

**1) ย่อ layout:** ย้ายกล่อง Bid ขึ้นใต้สถานะประมูลทันที · ย่อราคา/สถิติ/ร้าน/คำอธิบาย · ประวัติ bid เลื่อนในกล่องสูงสุด 120px · แชทเป็นลิงก์เล็ก  
**2) Auto-bid:** ปุ่ม `+ เปิด Auto-bid` — เปิดแล้วค่อยโชว์ช่องเพดาน · ไม่เปิด = ไม่ส่ง maxBid + ล้าง auto-bid เดิมตอนกด Bid (`clearAutoBid`)

---

## 2026-08-10 (18:42)

### หน้ารายละเอียดสินค้า/ประมูล — ลื้อโครงเก่า ทำใหม่แบบ Shopee

**ขอบเขต:** เฉพาะ `/marketplace/[id]` ไม่แตะหน้าอื่น  
**โครงใหม่:** การ์ดเดียว `.pd-panel` · ซ้าย `.pd-media` รูปหลักสัดส่วน 1:1 สูงสุด 450px + thumb แถว 80px · ขวา `.pd-info` ราคา/ประมูล/ร้าน/bid  
**เทคนิค:** ทิ้ง class `mkt-detail-*` ของหน้านี้ · ใช้ `pd-*` ชุดใหม่ทั้ง JSX+CSS · logic ซื้อ/ประมูลเดิมครบ

---

## 2026-08-10 (18:37)

### หน้ารายละเอียด — ขนาดรูปตามสไตล์ Shopee

**อ้างอิง:** ภาพ Shopee ที่ลูกค้าส่งมา  
**แก้:** รูปหลัก **450×450px** · gallery คอลัมน์ **478px** · grid `478px 1fr` · thumbnail **80×80px** · ล็อกพิกเซลตายตัว ไม่กินครึ่งจอ ไม่จิ๋ว

---

## 2026-08-10 (18:30)

### หน้ารายละเอียด — ล็อกรูปหลัก 210×210px ตายตัว

**ปัญหา:** รอบก่อนใช้ `fit-content` + `min(210px,100%)` + grid `auto` → วงจร CSS ยุบรูป / หรือคอลัมน์ยังกิน 50% จอ  
**แก้:** รูปหลัก `width/height: 210px` ตรงๆ · gallery `238px` · grid `238px 1fr` · ไม่ใช้ fit-content/% กับคอลัมน์ auto · thumb คืน 56px

---

## 2026-08-10 (18:08)

### หน้ารายละเอียด — แก้ layout รูปครึ่งหนึ่งที่เพี้ยน

**สาเหตุจริง:** ลดแค่ `max-width` รูปเป็น 210px แต่คอลัมน์ grid ยังกิน ~50% จอ (หรือ `210px` แยกจาก info) → รูปเล็กมุมซ้าย + พื้นที่ว่างกลางจอมหาศาล · thumb 28px เล็กเกิน  
**แก้:** grid `auto minmax(0,1fr)` — คอลัมน์รูปหดตามเนื้อหา (`width: fit-content`) · รูปหลัก 210px · info ขยายเต็มที่เหลือ · thumb 40px

---

## 2026-08-10 (17:55)

### หน้ารายละเอียด — ลดรูปครึ่งหนึ่งจริง (420→210px)

**ปัญหา:** จำกัด 280px แต่ breakpoint 1040px+ เท่านั้น · grid 768px ยัง 50-50 · กล่อง gallery ยืดสูงตามคอลัมน์ขวา → ดูเหมือนรูปไม่เล็กลง  
**แก้:** รูปหลัก + gallery **210px** (ครึ่งของ 420px) · grid 2 คอลตั้งแต่ 768px · `align-self: start` ไม่ให้กล่องรูปยืด · thumb 28px

---

## 2026-08-10 (17:49)

### หน้ารายละเอียดประมูล — ย้อน layout + ลดรูปอย่างเดียว

**แก้:** ลบ max-width container (920/980px) ที่ทำให้มีพื้นที่ว่างซ้าย-ขวา · คืน grid layout เดิม · ลดรูป gallery เป็น max 280px เท่านั้น ไม่แตะส่วนอื่น

---

## 2026-08-10 (17:43)

### หน้ารายละเอียดประมูล — ออกแบบใหม่ครั้งเดียวจบ

**Layout:** ชื่อสินค้าใหญ่บนสุด · แถบสรุป ราคา·เวลา·ผู้นำ·bid info บรรทัดเดียว · body 2 คอลัมน์ รูป ~280px + กล่อง Bid บนสุด · auto-bid + ประวัติ bid · รายละเอียด/ร้านย่อใน `<details>`  
**ไฟล์:** `marketplace/[id]/page.tsx` แยก layout ประมูล vs ตลาดปกติ · CSS class ใหม่ใน `globals.css`

---

## 2026-08-10 (17:35)

### แก้ layout หน้ารายละเอียดประมูล — ไม่เพี้ยน

จำกัดความกว้างทั้งหน้า ~980px · grid รูป/ข้อมูล 50-50 · รูป max 360px กลางคอลัมน์ · กล่อง bid ม่วงชัด

---

## 2026-08-10 (17:30)

### ประมูลของฉัน + Auto-bid (ราคาสูงสุดที่สู้)

**ประมูลของฉัน:** แท็บในตลาดประมูล · API `/api/auctions/mine` · แสดงสถานะ กำลังนำ/overbid/ชนะ/แพ้  
**Auto-bid:** ตาราง `auction_auto_bids` · ตั้ง max ในเมนู bid · ระบบสู้ให้อัตโนมัติทีละ increment  
**Migration:** `0032_auction_auto_bids.sql` — ต้องรันบน Supabase

---

## 2026-08-10 (17:25)

### หน้ารายละเอียดสินค้า — ลดขนาดรูปครึ่งหนึ่ง

gallery จำกัด max 420px · thumb 56px · grid คอลัมน์รูปแคบลง ให้ฝั่งข้อมูล/ bid ได้พื้นที่มากขึ้น

---

## 2026-08-10 (17:10)

### การ์ดประมูล compact + ฟอร์มลงประมูลสไตล์ Yahoo

การ์ดเล็กลง ~ครึ่ง (grid 2–6 คอล · รูป 4:3) · ชื่อสินค้าเด่นบนสุด · จำนวน bid ใหญ่ขึ้น · ฟอร์มลงประมูลใช้ mkt-yahoo-panel ให้ match หน้าตลาด

---

## 2026-08-10 (17:00)

### การ์ดประมูล — ลบข้อมูลซ้ำ + แถวข้อมูลรวมบรรทัดเดียว

ลบ badge ราคา/เวลาบนรูป (เหลือแค่「ประมูล」) · ราคา·เวลา·ผู้นำ แสดงใต้รูปที่เดียว · รวม จำนวนคน/bid/ที่อยู่/สภาพ เป็นบรรทัดเดียวไม่เหลือที่ว่าง

---

## 2026-08-10 (16:45)

### ตลาดประมูล — UI การ์ด + ค้นหาแบบ Yahoo

**การ์ดประมูล:** ราคา · เวลา · ผู้นำ แสดงใหญ่ 3 ช่อง สีแดง/ส้ม/เขียวกระตุ้น · ตัวเลขนับถอยหลังแยก digit · badge ราคาบนรูป  
**ค้นหา Yahoo:** dropdown หมวดหมู่ + ช่องค้นหา + ปุ่มแดง · หมวดหมู่เป็น chip grid ด้านล่าง  
**Layout:** เว้นขอบซ้าย ~1 ซม. · grid ประมูล 1–4 คอลัมน์ (การ์ดใหญ่ขึ้น)

---

## 2026-08-10 (16:30)

### ประมูล — เลือกระยะเวลา วัน/ชม./นาที + ตัวนับถอยหลังบนการ์ด

ลงประมูล: กำหนด วัน · ชม. · นาที เองได้ (สูงสุด 30 วัน) · การ์ดตลาดประมูลแสดงนับถอยหลังทุกวินาทีทั้งบนรูปและแถบม่วงใต้รูป

---

## 2026-08-10 (16:20)

### checkout ตลาด — กดดูหลักฐานแพ็ค + รีวิวอยู่บนสุด

หลักฐานแพ็คแตะเปิด lightbox ขนาดเต็ม · ฟอร์มดาวรีวิวย้ายไว้บนสุดก่อนสถานะ/หลักฐาน · หลังกดรับสินค้า scroll ขึ้นบนอัตโนมัติ

---

## 2026-08-10 (16:15)

### รีวิวดาวหลังรับสินค้า — หน้า checkout ตลาด

เดิม ReviewPanel อยู่แค่ห้องดีล — เพิ่ม `MarketplaceReviewBlock` ในหน้า `/cart/checkout/[id]` เมื่อสถานะ `completed` (ผู้ซื้อให้ดาวผู้ขาย + แพลตฟอร์ม)

---

## 2026-08-10 (16:10)

### แก้ UX แพ็คผู้ขาย + ผู้ซื้อเห็นหลักฐานแพ็ค

**ผู้ขาย:** ลบปุ่มซ้ำ — แท็บอื่นมีปุ่ม「แพ็คสินค้า」· แท็บรอแพ็คมีปุ่มเดียว「เริ่มแพ็คสินค้า」· ออเดอร์เดียวเปิดหน้าแพ็คอัตโนมัติ
**ผู้ซื้อ:** ส่ง `packingSteps` ไป checkout แล้ว — แสดงรูป/วิดีโอ 3 ขั้นในหน้าติดตามสถานะ
...
### แก้ 1012 สลิปซ้ำ — ถ้ามีข้อมูลสลิปและยอดตรงให้ผ่านอัตโนมัติ

สลิปอ่านได้ครั้งแรกแล้ว แต่กดตรวจซ้ำได้ 1012/1010 — ไม่ใช่ธนาคารช้า

---

## 2026-08-10 (15:40)

### แอดมินตลาด — แยกแสดงราคาสินค้า vs ยอดโอน (รวมขนส่ง)

SlipOK 1010 = ธนาคารยังไม่อัปเดต ไม่ใช่บั๊ก — รอแล้วตรวจซ้ำ

---

## 2026-08-10 (15:30)

### SLIPOK_BRANCH_ID — ใส่แค่เลข 69043 ไม่ใส่ URL

เพิ่ม `resolveSlipokBranchId()` ดึงเลขจาก URL อัตโนมัติถ้าวางผิด + ข้อความชัดในแอดมิน
...
**SlipOK**:
- `formatSlipokError()` ใน `slipok.ts` — แปลรหัส 1000–1014 + alias เช่น `Not Found` → 1011
- ใช้ใน auto-verify, LINE, reject_reason, หน้าการเงินแอดมิน

**แอดมินตลาด/ประมูล**:
- ปุ่ม **ยกเลิกคำสั่งซื้อ** — ล้างผู้ซื้อ/สลิป กลับ `posted` (ประกาศขายยังอยู่)
- ซ่อนปุ่ม **ลบดีล** สำหรับ `source=listing` (กันลบโพสต์ขายผู้ขาย)
- API `cancel_marketplace_checkout` — แจ้งผู้ซื้อในแอป

---

## 2026-08-10 (13:40)

### SlipOK ไม่ผ่าน — บันทึกเหตุผลในดีล + แจ้ง LINE

**พฤติกรรม** (เฉพาะตรวจสลิปอัตโนมัติจาก SlipOK):
1. ไม่ผ่าน → บันทึก `reject_reason` ในดีล เช่น `[สลิปผู้ซื้อ] ยอดเงินไม่ตรง ...`
2. แจ้ง LINE แอดมินทันทีพร้อมเหตุผลจาก SlipOK (ผ่าน `notifyAdminLineSlipResult`)
3. ผ่าน → ล้าง `reject_reason` + แจ้ง LINE ตามเดิม

**ไฟล์**: `src/app/api/_lib/slipAutoVerify.ts` — ย้อน logic แอดมินกดไม่ถูกต้องที่ทำผิด scope
...
**สาเหตุ**:
1. `admin/layout` ผูก `useEffect` กับ `pathname` → สลับเมนูแล้วรีเช็คชั้นรหัสผ่าน
2. `AuthGate` รีเช็คทุกครั้งที่เปลี่ยน path ทำให้ลูกถูกกระทบ

**แก้**:
1. เช็คสิทธิ์แอดมินครั้งเดียวตอนเข้า (ไม่ผูก pathname)
2. จำการปลดล็อกใน `localStorage` ผูก userId+deviceId (30 วัน) คู่กับ cookie
3. `AuthGate` — ถ้าเคยผ่าน gate แล้ว สลับหน้าไม่ unload/รีเซ็ตทั้งก้อน

---

## 2026-08-10 (12:36)

### Admin — จำเครื่อง 30 วัน ไม่ต้องใส่รหัสแผงทุกครั้ง

**พฤติกรรม**:
1. ล็อกอินบัญชีแอดมิน + ใส่รหัสแผงครั้งแรก → ออก cookie จำเครื่อง (HttpOnly) ผูก `userId + deviceId`
2. เข้า `/admin` ครั้งถัดไปบนอุปกรณ์เดิม + บัญชีเดิม → ข้ามฟอร์มรหัสผ่าน
3. เครื่องอื่น / บัญชีอื่น / หมดอายุ 30 วัน → ต้องใส่รหัสใหม่
4. กดออกจากระบบ → ล้าง cookie จำเครื่อง

**ไฟล์**:
- `src/lib/adminTrustedDevice.ts` — เซ็น/ตรวจ trust token
- `src/app/api/admin/verify-password/route.ts` — GET/POST/DELETE
- `src/app/admin/layout.tsx` — เช็ค trust ก่อนโชว์ฟอร์ม
...
**ทิศทาง**: เดสก์ท็อปกินความกว้างจอ · จอกว้างขึ้นแสดงข้อมูลมากขึ้น · มือถือคงแนวตั้ง

**แก้ไขหลัก**:
1. **`globals.css`** — ปลด `max-width` คอลัมน์แคบ (`.container`, `.dash-body`, `.pf-inner`, `.dr-*`, `.cart-*`, `.mkt-co-*`, `.svc-inner`, `.reg-body`, `.pay-inner`, `.dc-page`, หน้าร้าน ฯลฯ)
2. **Seller `/dashboard/seller`** — โครง `dash-shell` 2-pane (ป้ายร้านซ้าย sticky + คิวงานขวา) · กริดรายการ 2–3 คอลัมน์
3. **Middleman** — กริดการ์ดดีลหลายคอลัมน์บนจอกว้าง
4. **ตะกร้า/checkout** — 2 คอลัมน์ตั้งแต่แท็บเล็ต (~768) สรุปยอดขวา sticky
5. **ตลาด** — กริดสินค้าถึง 6 คอลัมน์บนจอกว้างมาก · ออเดอร์เป็นกริด
6. **โปรไฟล์/บริการ/ดีล/ชำระเงิน** — เต็มความกว้าง + จัดคู่ขนานลดสกรอลล์

**คงเดิม**: ล็อกอินการ์ดแคบ · มือถือ 1 คอลัมน์ / bottom tab

---

## 2026-08-10 (11:12)

### ตลาด/ประมูล — แพ็กของบนบอร์ดผู้ขาย + ขายแล้วขึ้นหน้าร้าน

**Flow**:
1. ผู้ขายลงขาย/ประมูล → ขึ้นตลาดตามประเภท
2. ผู้ซื้อกดซื้อ / ชนะประมูล → checkout (`/cart/checkout`) → โอนสลิป → แอดมินรอยืนยัน (หมวดตลาด)
3. สถานะบอร์ดผู้ขาย: **รอขาย → กำลังตรวจสอบ → กำลังแพ็ค → จัดส่งแล้ว → ขายแล้ว**
4. สลิปผ่าน → แพ็กกิ้งบน **`/dashboard/seller`** (3 ขั้น + เลขพัสดุ) ไม่ก๊อปห้องดีลทั้งดุ้น
5. ผู้ซื้อเห็นหลักฐานแพ็กทีละขั้นใน checkout/ตะกร้า
6. ขายแล้วเอาออกจากตลาด → ขึ้นบอร์ดผู้ขาย + หน้าร้านสาธารณะป้าย「ขายแล้ว」

**ไฟล์หลัก**:
- `SellerPackingPanel.tsx` — UI แพ็กบนบอร์ดผู้ขาย
- `marketplaceOrder.ts` — `isListingCheckoutOrder`, label สถานะร้าน, sold public
- `auctionSync.ts` — ชนะประมูลเข้า checkout แบบตลาด (ไม่ผ่านคนกลาง)
- checkout/orders API — รองรับประมูล + ส่ง packing evidence
- หน้าร้านสาธารณะ — เอาสถิติดำเนินงานออก (ซื้อสำเร็จ/ดีลสำเร็จ) · เพิ่มโซนขายแล้ว

---

## 2026-08-10 (02:42)

### LINE แอดมิน — แจ้งตรวจสลิปครั้งเดียว (ผ่าน/ไม่ผ่าน)

- ยกเลิกส่ง「ยืนยันรับเงิน」+「ตรวจสลิปอัตโนมัติ」แยก 2 ข้อความ
- **`notifyAdminLineSlipResult`** — ข้อความเดียวบอกผล + อนุมัติอัตโนมัติ (ถ้ามี)
- ตลาดใช้ยอด **`ราคา + ค่าขนส่ง`** ตอนตรวจสลิป
...
- **`/cart`** — แท็บ「รายการในตะกร้า」+「คำสั่งซื้อของฉัน」
- ไอคอน 🛒 Nav → `/cart?tab=orders`
- **`/marketplace/orders`** → redirect ไปตะกร้า
- checkout กลับตะกร้า ไม่ highlight แท็บ「ตลาด」
...
**งานที่ 1 — แยก component โอนเงินตลาด**:
- **`MarketplacePaymentSection.tsx`** — flow ตลาดเท่านั้น (ราคา + ขนส่ง)
- **`renderPaymentSection()`** — เหลือดีลผ่านกลาง/simple ไม่ปน `isMkt`

**งานที่ 2 — รายการสั่งซื้อ**:
- **`GET /api/marketplace/orders`** — คำสั่งซื้อผู้ซื้อ + `?count=1` สำหรับ badge
- **`/marketplace/orders`** — หน้ารายการสั่งซื้อ (กำลังดำเนินการ / สำเร็จ)
- **`MarketplaceOrdersIcon`** — ไอคอนตะกร้าใน Nav แสดงจำนวนที่ยังไม่จบ

---

## 2026-08-10 (01:40)

### ตลาด — ยอดโอนเหลือแค่ราคาสินค้า + ค่าขนส่ง

**สาเหตุ**: `renderPaymentSection` ใช้ `computeDealFees` แบบดีลผ่านกลาง (Escrow/คนกลาง/ตรวจสอบ)

**แก้ไข**: ตลาดใช้ `marketplaceBuyerPayAmount()` = ราคา (รวม GP แล้ว) + ค่าขนส่ง เท่านั้น

---

## 2026-08-10 (01:30)

### แก้หน้าโอนเงินตลาดว่าง (posted + buyer ไม่แสดง UI)

**สาเหตุ**: `renderPaymentSection()` เช็คแค่ `payment_pending` แต่ flow ใหม่คง `posted`

**แก้ไข**: ใช้ `isMarketplaceCheckoutActive()` แสดงเลขบัญชี/อัปสลิป + สถานะหัวข้อ「รอโอนเงิน」
...
**Flow ตลาด (ไม่รวมประมูล)**:
1. กดซื้อ → `payment_pending` + เปิดหน้าโอนเงิน (wizard ตลาด)
2. ยืนยันโอน → `payment_uploaded` รอเช็คสลิป (SlipOK/แอดมิน) — **ไม่ข้าม**
3. แอดมินยืนยัน → `packing` → จัดส่งตรงถึงผู้ซื้อ

**แก้ไข**:
1. **`marketplaceOrder.ts`** — helper ออเดอร์ตลาด / จัดส่งตรง
2. **`join_as_buyer`** — ตลาดข้ามเงื่อนไข → โอนเงินทันที
3. **`deal/[id]/page.tsx`** — wizard ตลาด + ค่าขนส่งในยอดโอน
4. **`lineAdminNotify.ts`** — ข้อความ/ลิงก์ `[กลางฮับ · ตลาดและประมูล]` → `?category=market&tab=confirm_pay`
5. **`dashboard/seller`** — แท็บ กำลังขาย / ขอแพคกิ้ง / เตรียมจัดส่ง / สำเร็จ / ประวัติ

---

## 2026-08-09 (14:34)

### หน้ารายละเอียดสินค้า + ค่าขนส่ง + แก้ไขประกาศในร้าน

**งานที่ 1 — หน้ารายละเอียดตลาด (`/marketplace/[id]`)**:
- เหลือแค่ **ซื้อทันที** (เข้าระบบซื้อขายผ่านกลาง), **แชทกับผู้ขาย**, ข้อความแจ้ง escrow
- เอาตะกร้า / นัดรับ / ฝากข้อความ / กล่องอธิบาย 2 แบบ ออก

**งานที่ 2 — ค่าขนส่ง + ขนส่งที่รองรับ**:
1. **`0029_listing_shipping.sql`** — `shipping_cost`, `shipping_providers`, `buyer_shipping_provider`
2. **ฟอร์มลงขาย/ประมูล** — ค่าขนส่ง + popup เลือกขนส่ง (`ShippingCarrierPicker`)
3. **แก้ไขประกาศ** — ปุ่ม ✏️ บนการ์ด `posted` + `PATCH update_listing`
4. **ฝั่งผู้ซื้อ** — แสดงค่าขนส่ง + เลือกขนส่งก่อนกดซื้อ

**งานฝั่งผู้ใช้**: รัน migration `0029_listing_shipping.sql` ใน Supabase

---

## 2026-08-09 (14:06)

### แก้ลงประกาศตลาดโดนบล็อกเมื่อปิด「ซื้อขายผ่านกลาง」

**สาเหตุ**: `POST /api/deals` เช็ค `tradeOnline` แทน `marketplace` เมื่อ `source=listing`

**แก้ไข**:
1. **`deals/route.ts`** — ลงประกาศตลาด/ประมูลเช็ค `marketplace` แยกจากดีลผ่านกลาง
2. **`serviceControls.ts`** — ปรับคำอธิบายสวิตช์ให้ชัด
...
**แก้ไข**:
1. แท็บ **ตลาดซื้อขาย** / **ตลาดประมูล** (เดิม ขายสินค้า / โซนประมูล)
2. ปรับ `.mkt-mode-tabs` ให้เด่นขึ้น — กรอบหนา, ตัวอักษรใหญ่/หนา, gradient + shadow ตอน active

---

## 2026-08-09 (00:45)

### รวมโซนประมูลกลับเข้าหน้าตลาด (ไม่แยก nav/หน้า)

**แก้ไข**:
1. **`/marketplace`** — สลับ zone **ขายสินค้า** / **โซนประมูล** ในหน้าเดียว (`?zone=auction`)
2. **`Site.tsx`** — ลบแท็บ nav **ประมูล** (เหลือแค่ **ตลาด**)
3. **`/marketplace/auctions`** — redirect → `/marketplace?zone=auction`
4. **`marketplace/[id]`** — ปุ่มกลับโซนประมูล → `?zone=auction`

---

## 2026-08-09 (00:35)

### เอาปุ่มลงสินค้า/ลงประมูลออกจากหน้าตลาดและประมูล

**แก้ไข**:
1. **`/marketplace`** — ลบปุ่ม「ลงขาย」จาก hero (เหลือ โซนประมูล + หาสินค้า)
2. **`/marketplace/auctions`** — ลบปุ่ม「ลงประมูล」จาก hero และ empty state (เหลือ กลับตลาดสินค้า / ดูตลาดสินค้า)

**หมายเหตุ**: ลงขาย/ลงประมูลทำได้จาก `/dashboard/seller` เท่านั้น

---

## 2026-08-09 (00:20)

### แยกหน้าประมูลออกจากตลาดสินค้า

**แก้ไข**:
1. **`/marketplace`** — แสดงเฉพาะสินค้าราคาตายตัว + ปุ่ม「โซนประมูล」
2. **`/marketplace/auctions`** — หน้าประมูลแยก (การ์ด countdown, ผู้นำ, จำนวน bid)
3. **`Site.tsx`** — เมนู nav แยกแท็บ **ตลาด** / **ประมูล**
4. หน้ารายละเอียด — ปุ่มกลับไปโซนที่ถูกต้อง

---

## 2026-08-08 (23:55)

### ระบบประมูลตลาด + ป๊อปอัพลงขาย

**โจทย์**:
1. กด「+ ลงขาย」→ ป๊อปอัพเลือก ลงขายสินค้า / ลงสินค้าประมูล
2. ประมูลจริง: นับถอยหลัง, ราคาเริ่ม, บิทครั้งละ, จำนวนผู้ประมูล, ใครนำ
3. การ์ดประมูลในตลาด + หน้ารายละเอียด bid ได้

**แก้ไข**:
1. **`0027_auction.sql`** — `deal_type auction`, `deal_auction`, `auction_bids`
2. **`src/lib/auction.ts`**, **`src/app/api/_lib/auctionSync.ts`**
3. **`POST /api/auctions/[id]/bid`**, ขยาย deals POST/GET
4. **`dashboard/seller`** — modal เลือกประเภท + ฟอร์มประมูล
5. **`marketplace`** — แท็บ สินค้า/ประมูล, การ์ด countdown
6. **`marketplace/[id]`** — panel ประมูล + bid + ประวัติ

**งานฝั่งผู้ใช้**: รัน migration `0027_auction.sql`
